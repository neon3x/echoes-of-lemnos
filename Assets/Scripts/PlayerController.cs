...
public class PlayerController : MonoBehaviour
{
        ...
        else
    {
        ...
        Plane groundPlane = new Plane(Vector3.forward, Vector3.zero); // Ground plane at Z = 0
        if (groundPlane.Raycast(ray, out float distance))
        {
            Vector3 worldPosition = ray.GetPoint(distance);
            toolIndicator.position = new Vector3(worldPosition.x, worldPosition.y, 0f);
        }

        // Clamp tool indicator to tool range
